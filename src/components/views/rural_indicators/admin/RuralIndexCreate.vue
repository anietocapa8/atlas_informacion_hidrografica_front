<template>
    ...
    <div v-if="isValidType && this.selectedTypeOptions.value == RURAL_INDICATOR_TYPES.INDEX">
      <b-form @submit.prevent="submit">
        ...
        <b-row class="pt-2">
          <b-form-group label-class="font-weight-bold" class="w-100">
            <template slot="label">{{ $t( 'rural_indicators.admin.index_name' ) }}<span class="required"> *</span></template>
            <b-form-input type="text" name="indicatorName" :data-vv-as="$t('rural_indicators.admin.index_name')" 
        ...
        <b-row class="pt-2">
          <b-form-group label-class="font-weight-bold" class="w-100">
            ...
            <map-search v-if="currentIndicator.map" name="ruralIndicatorMap" v-model="currentIndicator.map.url" :placeholder="$t( 'rural_indicators.admin.map_placeholder' )"
                class="bg-white" v-validate="{ required: true }" :data-vv-as="$t( 'rural_indicators.admin.map' )" data-vv-scope="form" :tag="ARCGIS_TAGS.INDEXES" />
            <span v-if="errors.has( 'form.ruralIndicatorMap' )" class="error"> {{ errors.first( 'form.ruralIndicatorMap' ) }} </span>
          </b-form-group>
        </b-row>
        <b-row class="pt-2">
          <b-form-group label-class="font-weight-bold" class="w-100">
            <template slot="label">{{ $t( 'rural_indicators.admin.index_summary' ) }}<span class="required"> *</span></template>
            <b-form-textarea name="indicatorSummary" class="inline-control" :rows="3" :max-rows="6"
              v-model="currentIndicator.summary" v-validate="{ required: true, max: 280 }" :data-vv-as="$t( 'rural_indicators.admin.index_summary' )"
              data-vv-scope="form">
            </b-form-textarea>
        ...
        <b-row class="pt-2">
          <b-form-group label-class="font-weight-bold" class="w-100">
            <template slot="label" class="font-weight-bold">{{ $t('rural_indicators.admin.index_description') }}<span class="required"> *</span></template>
            <rich-text :options="customToolbar" @changeValue="changeDescriptionContent" :newContent="currentIndicator.description"></rich-text>
            <span class="error" v-if="currentIndicator.description == null || currentIndicator.description == ''">
              {{ $t('rural_indicators.admin.index_description_error' )}}
            </span> 
          </b-form-group>
        </b-row>
        <b-row class="pt-2">
          <b-form-group label-class="font-weight-bold" class="w-100 negative-mt">
            <template slot="label">{{ $t('rural_indicators.admin.index_components') }}<span class="required"> *</span></template>
            <rich-text :options="customToolbar" @changeValue="changeComponentsContent" :newContent="currentIndicator.components"></rich-text>
            <span class="error" v-if="currentIndicator.components == null || currentIndicator.components == ''">
              {{ $t('rural_indicators.admin.index_components_error' )}}
            </span> 
          </b-form-group>
        </b-row>
        <!-- Sources, documents and links box -->
        <b-row class="bordered-container pt-2 negative-mt" align-v="center">
          <b-col cols="12">
          ...
          <b-col cols="12">
            <b-form-group>
              ...
              <rich-text :options="customToolbar" @changeValue="changeSourceContent" :newContent="currentIndicator.source"></rich-text>
        ...
        <b-row align-h="center" class="pt-5">
          ...
          <sipsder-btn btnType="accept" type="submit" :text="$t('rural_indicators.admin.indicator_save')"></sipsder-btn>
        </b-row>
      </b-form>
    </div>
    ...
    <b-modal ref="errorSubmitFormModal" :title="$t('rural_indicators.admin.popup.title')" hideFooter>
      <b-row align-h="center">
        <b-col cols="10">
          {{ $t('rural_indicators.admin.popup.index_error') }}
        </b-col>
      </b-row>
      <b-row class="btn-container" align-h="center">
    ...
    <b-modal ref="successModal" :title="$t('rural_indicators.admin.popup.title')" hideFooter
      ...
      <b-row align-h="center">
        <b-col cols="10">
          <div>
            {{ $t('rural_indicators.admin.popup.success1_index') }}
          </div>
          <div class="pt-3">
...
export default {
  ...
  async mounted() {
    this.selectedTypeOptions = this.typeOptions.filter( item => item.value == parseInt(RURAL_INDICATOR_TYPES.INDEX))[ 0 ];
    this.ruralDocuments = await this.getRuralDocuments( );
    this.geographicCoverage = await this.getGeographicCoverages( );
    if ( this.isEditing ) {
      this.currentIndicator = await this.getRuralIndex( this.$route.params.id );
      this.selectedTypeOptions = this.typeOptions.filter( item => item.value == parseInt(this.currentIndicator.type))[ 0 ];
      // this.currentIndicator.map = this.maps.filter( map => map.url == this.currentIndicator.map.url)[ 0 ];
  ...
  methods: {
    ...
    async getRuralIndex( id ) {
      try {
        let response = await VAPI.get( `/${SERVICE_NAMES.RURAL_INDICATORS}/admin/index/${id}` );
        return response.data.data;
      }
      catch( err ) { 
        console.error( 'Error fetching rural index ', err )
      }
    },
    changeSourceContent( value ){
      this.currentIndicator.source = value
    },
    changeDescriptionContent( value ) {
      this.currentIndicator.description = value
    },
    changeComponentsContent( value ) {
      this.currentIndicator.components = value
    },
    ...
    assemblePayload( ) {
      let payload = {
        ...
        description: this.currentIndicator.description,
        components: this.currentIndicator.components,
    ...
    validateForm() {
      let isValid = this.isValidType && this.currentIndicator.tags != null 
        && this.currentIndicator.tags.length > 0 && this.currentIndicator.responsible != null && this.currentIndicator.dice != null 
        && this.currentIndicator.source != null && this.currentIndicator.source != ''
        && this.currentIndicator.description != null && this.currentIndicator.description != ''
        && this.currentIndicator.components != null && this.currentIndicator.components != ''
    ...
    async submitIndicator( payload ) {
      ...
      path += !this.isEditing ? `index/create` : `index/update/${this.$route.params.id}`
        ...
        try {
          ...
          .then( 
            ( success ) => {
              ...
              console.error( 'Error trying to post index', error );
        ...
        try {
          ...
          .then( 
            ( success ) => {
              ...
              console.error( 'Error trying to put index', error );
              this.showModal( 'errorSubmitFormModal' );
            }
          );      
        }
