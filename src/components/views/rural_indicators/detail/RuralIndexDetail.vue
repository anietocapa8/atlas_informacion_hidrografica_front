<template>
    <div class="rural-detail">
        <div class="feed-title">{{ $t( 'rural_indicators.detail.index.title' ) }}</div>
        <div v-if="figure">
            <b-row class="div-90 pt-5 pb-2">
                ...
                <b-col cols="2" lg="2" xs="12" sm="12">
                    <b-row align-h="end" align-v="center">
                        <sipsder-btn btnType="edit_pen" :text="$t( 'rural_indicators.detail.index.edit' )" @onClick="$router.push( { name: 'RuralIndexCreate', params: { id: $route.params.id } } )" v-if="isAdministrator"></sipsder-btn>
                    </b-row>
                </b-col>
            </b-row>
            <b-row class="div-90 pt-5 pb-2">
            ...
            <b-row class="div-90 pb-5">
                <div class="title">{{ $t( 'rural_indicators.detail.index.curriculum' ) }}</div>
            </b-row>
            <b-row>
                <div class="text div-90">
                    <div class="A">
                        <div class="tab-design" :class="{active : tabIndex==0 }" @click="setTabIndex(0)"><div><span>{{ $t( 'rural_indicators.detail.index.description' ) }}</span></div></div>
                        <div class="tab-design" :class="{active : tabIndex==1 }" @click="setTabIndex(1)"><div><span>{{ $t( 'rural_indicators.detail.index.components' ) }}</span></div></div>
                        <div class="tab-design" :class="{active : tabIndex==2 }" @click="setTabIndex(2)"><div><span>{{ $t( 'rural_indicators.detail.figure.sources' ) }}</span></div></div>
                        <div class="tab-design" :class="{active : tabIndex==3 }" @click="setTabIndex(3)"><div><span>{{ $t( 'rural_indicators.detail.figure.responsible' ) }}</span></div></div>
                    </div>
                    <div v-if="tabIndex == 0" class="B">
                        <b-row class="py-3 px-5">
                            <div v-html="figure.generalDescription"></div>
                        </b-row>
                    </div>
                    <div v-if="tabIndex == 1" class="B">
                        <b-row class="py-3 px-5">
                            <div v-html="figure.components"></div>
                        </b-row>
                    </div>
            ...
            <b-row class="div-90 pt-5 pb-2">
                ...
                <note-rich-text :module="MODULES.RURAL_INDICATORS" keyVal="index"></note-rich-text>
            </b-row>
...
export default {
  ...
  async beforeMount( ) {
    this.figure = await this.getRuralIndex( this.$route.params.id );
  },
  data( ) {
    return {
      SERVICE_NAMES,
  ...
  methods: {
    async getRuralIndex( id ) {
      try {
        const response = await VAPI.get( `/${SERVICE_NAMES.RURAL_INDICATORS}/detail/index/${id}` );
