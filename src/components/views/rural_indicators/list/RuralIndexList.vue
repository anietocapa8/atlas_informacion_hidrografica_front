    ...
    <div>
        <div class="inner-legal-note">
            <note-rich-text :showEdition="isAdministrator" :module="MODULES.RURAL_INDICATORS" 
                keyVal="index" :textPenEdition="$t('rural_indicators.index.edit_legal_note')">
            </note-rich-text>
        </div>
        ...
        <div class="w-100">
            <div class="search-box mb-5">
                <b-row>
                    ...
                    <b-col cols="3" v-if="isAdministrator">
                        <b-form-group class="pt-3">
                            <b-row align-v="center">
              ...
              <b-row class="px-5 py-3" align-v="center" v-else>
                <b-row v-for="(index, i) in indexes" :key="i" class="w-100">
                    <b-col cols="2" class="p-0">
                      <map-preview :value="index.mapUrl" display-simple />
                    </b-col>
                    <b-col cols="10">
                        <b-row class="px-3" align-v="center">
                            <b-col>
                                ...
                                <router-link :to="{ name: 'RuralIndexDetail' , params: { id: index.id }}" class="indicator-name">{{ index.name }}</router-link>
                                <div><strong>{{ $t('rural_indicators.admin.summary') }}:</strong> {{ index.summary }}</div>
                                <div><strong>{{ $t('rural_indicators.admin.geographic_coverage') }}:</strong> {{ index.geographic }}, <strong>{{ $t('rural_indicators.index.update') }}:</strong> {{ index.updateDate | moment("DD/MM/YYYY") }}</div>
                            </b-col>
...
export default {
  ...
  methods: {
    ...
    async getIndexes( ) {
      try {
        const response = await VAPI.get( `/${SERVICE_NAMES.RURAL_INDICATORS}/all/indexes${this.queryString}` );
        return response;
      }
      catch( error ) {
